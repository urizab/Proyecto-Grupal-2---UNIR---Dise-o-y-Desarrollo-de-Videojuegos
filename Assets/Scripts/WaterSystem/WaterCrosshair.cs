...
public class WaterCrosshair : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Animación del anillo: pulso en reposo, comprimido al disparar
        if (_ringRT != null)
        {
            float targetScale = _isFiring
                ? 0.85f
                : 1f + Mathf.Sin(Time.time * pulseSpeed) * pulseAmount;

            _ringRT.localScale = Vector3.Lerp(
                _ringRT.localScale,
    ...
    // ── API PÚBLICA — llamado desde WaterHose
